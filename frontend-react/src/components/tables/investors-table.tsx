...
import type { IInvestor } from "../../services/admin/adminInvestors/adminInvestors.types";
...
const kycStyle: Record<string, string> = {
  UNVERIFIED: "text-gray-500 bg-gray-50 border-gray-300",
          ...
          <table className="w-full text-sm min-w-175">
            <thead>
              <tr className="bg-indigo-600 text-white">
                ...
                <th className="text-center px-5 py-3 font-medium">KYC</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {data.length === 0 ? (
                ...
                data.map((investor) => (
                  <tr key={investor._id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-5 py-3.5">
                      ...
                      <p className="text-xs text-indigo-500">@{investor.username}</p>
                    </td>
                    ...
                    <td className="px-5 py-3.5 text-center">
                      <span className={`text-xs px-2.5 py-0.5 rounded-full capitalize font-medium border ${kycStyle[investor.kycStatus] || ""}`}>
                        {investor.kycStatus}
                      </span>
                    </td>
                    <td className="px-5 py-3.5 text-xs text-gray-600 whitespace-nowrap">
                      <p>{fmtDate(investor.createdAt)}</p>
                      <p className="text-gray-400">{timeAgo(investor.createdAt)}</p>
                    </td>
                    <td className="px-5 py-3.5 text-center">
                      <Link
                        to={`/investors/${investor.username}`}
