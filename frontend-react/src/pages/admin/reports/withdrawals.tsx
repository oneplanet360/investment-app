        ...
        <div className="bg-white rounded-lg shadow-sm p-5">
          ...
          <div className="space-y-3">
            {[
              {
                label: "Approved",
                value: investor.approved.total,
                count: investor.approved.count,
                color: "bg-green-500",
              },
              {
                label: "Pending",
                value: investor.pending.total,
                count: investor.pending.count,
                color: "bg-yellow-400",
              },
              {
                label: "Rejected",
                value: investor.rejected.total,
                count: investor.rejected.count,
                color: "bg-red-400",
              },
            ].map(({ label, value, count, color }) => {
              const total =
                investor.approved.total +
                investor.pending.total +
                investor.rejected.total;
        ...
        <div className="bg-white rounded-lg shadow-sm p-5">
          ...
          <div className="space-y-3">
            {[
              {
                label: "Approved",
                value: agent.approved.total,
                count: agent.approved.count,
                color: "bg-indigo-500",
              },
              {
                label: "Pending",
                value: agent.pending.total,
                count: agent.pending.count,
                color: "bg-yellow-400",
              },
              {
                label: "Rejected",
                value: agent.rejected.total,
                count: agent.rejected.count,
                color: "bg-red-400",
              },
            ].map(({ label, value, count, color }) => {
              const total =
                agent.approved.total +
                agent.pending.total +
                agent.rejected.total;
              const pct = total ? Math.round((value / total) * 100) : 0;
              return (
                <div key={label}>
                  <div className="flex justify-between text-sm mb-1">
