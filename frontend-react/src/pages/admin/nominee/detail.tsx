...
import { useAdminNomineeDetailQuery } from "../../../services/admin/adminNominee/adminNominee.queries";
import { useUpdateNomineeStatusMutation } from "../../../services/admin/adminNominee/adminNominee.queries";
...
export default function NomineeDetail() {
  ...
  const { data: nominee, isLoading, error } = useAdminNomineeDetailQuery(id || "");
  const { mutate: updateStatus, isPending } = useUpdateNomineeStatusMutation();
  ...
  if (isLoading) {
    return (
      <div className="min-h-full bg-[var(--theme-bg)] p-6 flex flex-col items-center justify-center gap-3">
        ...
        <p className="text-gray-500 text-sm">Loading Nominee details...</p>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-full bg-[var(--theme-bg)] p-6 flex flex-col items-center justify-center gap-3">
        <p className="text-gray-500">
          Nominee submission not found or failed to load.
        </p>
        <Link
          to="/admin/nominee/all"
  ...
  const handleApprove = () => {
    updateStatus(
      { id: nominee._id, status: "APPROVED" },
      {
        onSuccess: () => {
  ...
  const handleReject = () => {
    ...
    updateStatus(
      { id: nominee._id, status: "REJECTED", remarks: rejectReason },
      {
        onSuccess: () => {
          setShowRejectForm(false);
        },
  ...
  return (
    <div className="min-h-full bg-[var(--theme-bg)] p-4 sm:p-6 space-y-5">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-base font-semibold text-gray-700">Nominee Review</h1>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-lg shadow-sm p-5 space-y-4">
          ...
          <div className="divide-y divide-gray-50 text-sm">
            {[
              ["Full Name", nominee.userId.name],
              ["Username", `@${nominee.userId.username}`],
              ["User Type", nominee.userId.role.toLowerCase()],
              ["Country", nominee.userId.country || "—"],
              ["Address", nominee.userId.address || "—"],
              ["Submitted", fmt(nominee.createdAt)],
              ...(nominee.status !== "PENDING"
                ? [["Reviewed", fmt(nominee.updatedAt)]]
                : []),
            ].map(([label, value]) => (
              <div
                key={label}
            ...
            <div className="flex items-center justify-between py-2.5">
              ...
              <span
                className={`text-xs px-2.5 py-0.5 rounded-full capitalize font-medium border ${statusStyle[nominee.status] || statusStyle.UNVERIFIED}`}
              >
                {nominee.status.toLowerCase()}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm p-5 space-y-4">
          ...
          <div className="divide-y divide-gray-50 text-sm">
            {[
              ["Nominee Name", nominee.nomineeName],
              ["Relation", nominee.relation],
              ["Document Type", nominee.documentType],
              ["Document Number", nominee.documentNumber || "—"],
            ].map(([label, value]) => (
              <div
                key={label}
                className="flex items-start justify-between py-2.5 gap-2"
          ...
          {nominee.status === "PENDING" && (
            <div className="pt-2 space-y-2">
              ...
              >
                <XCircle size={16} /> Reject Nominee
              </button>
              {showRejectForm && (
        ...
        <div className="bg-white rounded-lg shadow-sm p-5">
          ...
          <div className="space-y-3">
            <div>
              ...
              <img
                src={nominee.documentFrontUrl}
                alt="Document Front"
                className="w-full rounded-lg border border-gray-100 object-cover"
              />
            </div>
            {nominee.documentBackUrl && (
              <div>
                <p className="text-xs text-gray-500 mb-1.5 font-medium">
                  Back Side
                </p>
                <img
                  src={nominee.documentBackUrl}
                  alt="Document Back"
                  className="w-full rounded-lg border border-gray-100 object-cover"
                />
              </div>
