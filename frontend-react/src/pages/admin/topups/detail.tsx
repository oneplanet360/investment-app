...
export default function TopUpDetail() {
  ...
  const rows: [string, React.ReactNode][] = [
    ["Transaction ID", <span className="font-mono font-semibold text-xs">{topUp.trxId}</span>],
    ["Status", <span className={`text-xs px-2.5 py-0.5 rounded-full capitalize font-medium border ${statusStyle[topUp.status]}`}>{topUp.status}</span>],
    ["Full Name", topUp.fullName],
    ["Username", <span className="text-indigo-500">@{topUp.username}</span>],
    ["Investment Trx ID", <span className="font-mono text-xs text-gray-600">{topUp.investmentTrxId}</span>],
    ["Top-up Amount", <span className="font-semibold text-gray-800">${topUp.amount.toLocaleString("en-US", { minimumFractionDigits: 2 })} USD</span>],
  ...
  return (
    <div className="min-h-full bg-[var(--theme-bg)] p-4 sm:p-6 space-y-5">
      ...
      <div className="max-w-xl bg-white rounded-lg shadow-sm p-5">
        <h2 className="text-sm font-semibold text-gray-700 mb-4">Top-up Information</h2>
