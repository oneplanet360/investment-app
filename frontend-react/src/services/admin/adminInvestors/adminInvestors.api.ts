...
import type {
  ...
  ResetInvestorPasswordPayload,
} from "./adminInvestors.types";
...
export const getInvestorsFn = async (
  ...
  const response = await axiosInstance.get(
    `${ADMIN_INVESTORS_URL}?${params.toString()}`,
  );
  return { data: response.data.data, meta: response.data.meta };
};

export const resetInvestorPasswordFn = async (
  payload: ResetInvestorPasswordPayload,
): Promise<void> => {
  await axiosInstance.put(
    `${ADMIN_INVESTORS_URL}/${payload.investorId}/password-reset`,
    {
...
export const getAdminInvestorDetailApi = async (
  username: string,
