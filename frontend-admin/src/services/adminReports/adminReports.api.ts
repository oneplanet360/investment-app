...
export const getAdminInvestmentReportApi = async (): Promise<InvestmentReportStats> => {
...
export const getAdminWithdrawalReportApi = async (): Promise<WithdrawalReportStats> => {
  const { data } = await axiosInstance.get('/admin/reports/withdrawals');
  return data.data;
};

export const getAdminCommissionReportApi = async (): Promise<CommissionReportStats> => {
