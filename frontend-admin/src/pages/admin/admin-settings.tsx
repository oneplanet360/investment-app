...
import { useAdminSettingsMutation } from "../../services/adminSettings/adminSettings.mutation";
import type { AdminSettingsPayload } from "../../services/adminSettings/adminSettings.types";
...
const schema = z.object({
  ...
  primaryColor: z.string().regex(/^#[0-9A-Fa-f]{6}$/, "Invalid hex").or(z.literal("")),
  backgroundColor: z.string().regex(/^#[0-9A-Fa-f]{6}$/, "Invalid hex").or(z.literal("")),
  sidebarColor: z.string().regex(/^#[0-9A-Fa-f]{6}$/, "Invalid hex").or(z.literal("")),
...
const Field = ({ label, desc, error, children }: { label: string; desc?: string; error?: string; children: React.ReactNode }) => (
  <div>
...
export default function AdminSettings() {
  ...
  const handleFileUpload = (k: keyof FormData, e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="lg:col-span-2 space-y-5">
          ...
          <Section icon={<Layout size={16} />} title="Brand Identity">
            <div className="space-y-4">
              <Field label="App Name" desc="The name displayed in the browser tab and sidebar if no logo is provided." error={errors.appName?.message}>
                <input className={inputCls} {...register("appName")} placeholder="e.g. Finzip" />
              </Field>
              <Field label="Frontend URL" desc="The URL where the client-facing application is hosted." error={errors.frontendUrl?.message}>
                <input className={inputCls} type="url" {...register("frontendUrl")} placeholder="https://yourclienturl.com" />
              </Field>
            </div>
          </Section>
          ...
          <Section icon={<Palette size={16} />} title="Theme Colors">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Field label="Primary Color" desc="Main color for buttons and active links." error={errors.primaryColor?.message}>
                <div className="flex items-center gap-3">
                  <input type="color" value={app.primaryColor || "#4f46e5"} onChange={(e) => setValue("primaryColor", e.target.value, { shouldValidate: true, shouldDirty: true })} className="w-10 h-10 rounded cursor-pointer border-0 p-0" />
                  <input type="text" className={inputCls} {...register("primaryColor")} placeholder="#4f46e5" maxLength={7} />
                </div>
              </Field>
              <Field label="Background Color" desc="Global app background." error={errors.backgroundColor?.message}>
                <div className="flex items-center gap-3">
                  <input type="color" value={app.backgroundColor || "#f0f2f8"} onChange={(e) => setValue("backgroundColor", e.target.value, { shouldValidate: true, shouldDirty: true })} className="w-10 h-10 rounded cursor-pointer border-0 p-0" />
                  <input type="text" className={inputCls} {...register("backgroundColor")} placeholder="#f0f2f8" maxLength={7} />
                </div>
              </Field>
              <Field label="Sidebar Color" desc="Sidebar background color." error={errors.sidebarColor?.message}>
                <div className="flex items-center gap-3">
                  <input type="color" value={app.sidebarColor || "#ffffff"} onChange={(e) => setValue("sidebarColor", e.target.value, { shouldValidate: true, shouldDirty: true })} className="w-10 h-10 rounded cursor-pointer border-0 p-0" />
                  <input type="text" className={inputCls} {...register("sidebarColor")} placeholder="#ffffff" maxLength={7} />
                </div>
              </Field>
              <Field label="Font Family" desc="Global typography font." error={errors.fontFamily?.message}>
                <input className={inputCls} {...register("fontFamily")} placeholder="Inter, sans-serif" />
              </Field>
            </div>
          </Section>
          ...
          <div className="bg-white rounded-lg shadow-sm p-6 sticky top-6">
            ...
            <div className="space-y-6">
              <div style={{ fontFamily: app.fontFamily || "inherit" }}>
                ...
                <div className="flex items-center gap-2 p-3 rounded-lg border border-gray-200" style={{ backgroundColor: app.sidebarColor || "#111827" }}>
                  {app.logoUrl ? (
                    <img src={app.logoUrl} alt="Logo" className="h-6 object-contain" />
                  ) : (
                    <span className="font-bold text-lg tracking-tight" style={{ color: app.primaryColor || "#ffffff" }}>
              ...
              <div>
                ...
                <div className="flex items-center gap-2 p-2 bg-gray-100 rounded-t-lg border-b border-gray-300 w-48">
                  ...
                  <span className="text-xs text-gray-700 truncate">{app.appName || "App Name"} - Admin</span>
                </div>
              </div>
