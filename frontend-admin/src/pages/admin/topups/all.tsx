...
export default function AllTopUps() {
  ...
  const filtered = useMemo(() => {
    ...
    return topUps.filter(
      (t) => t.username.toLowerCase().includes(q) || t.trxId.toLowerCase().includes(q) || t.investmentTrxId.toLowerCase().includes(q)
    );
  }, [query]);
    ...
    <div className="min-h-full bg-[var(--theme-bg)] p-4 sm:p-6 space-y-4">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          { label: "Total Records",        value: topUps.length,                                              color: "text-indigo-600" },
          { label: "Completed",            value: topUps.filter(t => t.status === "completed").length,         color: "text-green-600" },
          { label: "Pending",              value: topUps.filter(t => t.status === "pending").length,           color: "text-yellow-600" },
        ].map((c) => (
          ...
          <table className="w-full text-sm min-w-[800px]">
            <thead>
              <tr className="bg-indigo-600 text-white">
                ...
                <th className="text-left px-5 py-3 font-medium">ROI Cycle Start</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {slice.length === 0 ? (
                ...
                <tr key={t.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-5 py-3.5 font-mono text-xs font-semibold text-indigo-500">{t.trxId}</td>
                  <td className="px-5 py-3.5">
                    <p className="font-semibold text-gray-800 text-sm">{t.fullName}</p>
                    <p className="text-xs text-indigo-500">@{t.username}</p>
                  </td>
                  <td className="px-5 py-3.5 font-mono text-xs text-gray-500">{t.investmentTrxId}</td>
                  <td className="px-5 py-3.5 text-right font-semibold text-gray-700">${t.amount.toLocaleString("en-US", { minimumFractionDigits: 2 })} USD</td>
                  <td className="px-5 py-3.5 text-xs text-gray-500 whitespace-nowrap">{fmt(t.roiCycleStart)}</td>
                  <td className="px-5 py-3.5 text-xs text-gray-500 whitespace-nowrap">{fmt(t.createdAt)}</td>
                  <td className="px-5 py-3.5 text-center">
                    <span className={`text-xs px-2.5 py-0.5 rounded-full capitalize font-medium border ${statusStyle[t.status]}`}>
                      {t.status}
                    </span>
                  </td>
                  <td className="px-5 py-3.5 text-center">
                    <Link to={`/topups/detail/${t.trxId}`} className="inline-flex items-center text-xs font-medium text-indigo-600 border border-indigo-300 rounded px-3 py-1.5 hover:bg-indigo-50 transition-colors">
                      Details
                    </Link>
                  </td>
                </tr>
