...
export default function CommissionsLog() {
  ...
  const filtered = useMemo(() => {
    let base = commissionLogs;
    if (levelFilter !== "all") base = base.filter((c) => String(c.level) === levelFilter);
    const q = query.toLowerCase().trim();
    if (!q) return base;
    return base.filter(
      (c) => c.agentUsername.toLowerCase().includes(q) || c.trxId.toLowerCase().includes(q) || c.fromInvestorUsername.toLowerCase().includes(q)
    );
  }, [query, levelFilter]);
        ...
        {([1,2,3,4] as const).map((lvl) => {
          ...
          return (
            <div key={lvl} className="bg-white rounded-lg shadow-sm p-4">
              ...
              <p className="text-xl font-bold mt-1 text-indigo-600">${total.toFixed(2)}</p>
            </div>
          );
        })}
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm overflow-hidden">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-5 py-4 border-b border-gray-100">
          <div>
            ...
            <p className="text-xs text-gray-400">Total paid: <strong className="text-emerald-600">${totalPaid.toFixed(2)}</strong></p>
          </div>
              ...
              {slice.length === 0 ? (
                ...
                <tr key={c.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-5 py-3.5">
                    <p className="font-semibold text-gray-800 text-sm">{c.agentFullName}</p>
                    <p className="text-xs text-indigo-500">@{c.agentUsername}</p>
                  </td>
                  <td className="px-5 py-3.5 text-xs text-gray-500">@{c.fromInvestorUsername}</td>
                  <td className="px-5 py-3.5 font-mono text-xs text-gray-500">{c.investmentTrxId}</td>
                  <td className="px-5 py-3.5 text-center">
                    <span className={`text-xs px-2.5 py-0.5 rounded-full font-semibold border ${levelColors[c.level]}`}>
                      L{c.level}
                    </span>
                  </td>
                  ...
                  <td className="px-5 py-3.5 text-xs text-gray-500 whitespace-nowrap">{fmt(c.creditedAt)}</td>
                </tr>
              ))}
            </tbody>
          </table>
