...
import { useResetAgentPasswordMutation } from "../../../services/adminAgents/adminAgents.mutation";
...
export default function AgentResetPassword() {
  ...
  const { data, isLoading: isSearching } = useAgentsQuery(1, 5, debouncedQuery);
  const results = data?.data || [];

  const { mutate: resetPassword, isPending } = useResetAgentPasswordMutation();
              ...
              <div className="absolute top-full left-0 right-0 bg-white border border-gray-200 rounded-b shadow-lg z-10 max-h-48 overflow-y-auto">
                {results.length > 0 ? (
                  ...
                  <div className="px-4 py-3 text-sm text-gray-500 text-center">
                    {isSearching ? "Searching..." : "No agents found"}
                  </div>
                )}
              </div>
            )}
