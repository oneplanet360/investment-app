...
import { useAdminRoiLogs } from "../../../services/adminRoi/adminRoi.query";
...
export default function RoiLog() {
  ...
  const meta = response?.meta as any || { total: 0, totalPages: 1, stats: { creditedCount: 0, pendingCount: 0, totalPaid: 0 } };
  const stats = meta.stats || { creditedCount: 0, pendingCount: 0, totalPaid: 0 };
  ...
  return (
    <div className="min-h-full bg-(--theme-bg) p-4 sm:p-6 space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          { label: "Total ROI Paid", value: `$${stats.totalPaid.toFixed(2)}`, color: "text-emerald-600", bg: "bg-emerald-50" },
          { label: "Total Records",   value: meta.total,              color: "text-indigo-600",  bg: "bg-indigo-50"  },
          { label: "Credited",        value: stats.creditedCount, color: "text-green-600", bg: "bg-green-50" },
          { label: "Pending",         value: stats.pendingCount,  color: "text-yellow-600", bg: "bg-yellow-50" },
        ].map((c) => (
            ...
            <tbody className="divide-y divide-gray-50">
              {isLoading ? (
                ...
                <tr key={r._id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-5 py-3.5 font-mono text-xs text-gray-500">{r.investmentId?.trxId}</td>
                  <td className="px-5 py-3.5">
                    <p className="font-semibold text-gray-800 text-sm">{r.investorId?.firstName || r.investorId?.lastName ? `${r.investorId?.firstName} ${r.investorId?.lastName}` : r.investorId?.name}</p>
                    <p className="text-xs text-indigo-500">@{r.investorId?.username}</p>
                  </td>
                  ...
                  <td className="px-5 py-3.5 text-xs text-gray-500 whitespace-nowrap">{fmt(r.createdAt)}</td>
                  <td className="px-5 py-3.5 text-center">
